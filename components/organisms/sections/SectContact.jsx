"use client";
import { motion } from "motion/react";
import Text from "@/components/atoms/Text";
import { Send } from "lucide-react";

import Link from "next/link";

const SectContact = () => {
  return (
    <section id="contact" className="w-full h-[70vh] bg-stone-300 ">
      <div className="h-full flex flex-col justify-center items-center p-10 gap-8">
        <Text style={"text-4xl font-bold"} text={"Get in touch"} />
        <Text
          style={
            "text-lg text-gray-600 font-outline text-center font-bold whitespace-pre-wrap"
          }
          text={`Have a project in mind or just want to chat?\n Feel free to reach out through any of the platforms below!`}
        />
        <div className="flex gap-6.5 items-center">
          <Link href={"https://mail.google.com/"}>
            <motion.div
              whileHover="hover"
              variants={{
                hover: {
                  backgroundColor: "#E0E0E0",
                },
              }}
              className="flex items-center px-4 py-2 gap-4.75 border border-gray-500 rounded-xl bg-white"
            >
              <motion.div
                variants={{
                  hover: {
                    x: 8,
                    rotate: 10,
                    transition: { duration: 0.3 },
                  },
                }}
              >
                <Send size={24} />
              </motion.div>
              <div className="font-bold">Email me</div>
            </motion.div>
          </Link>

          <Link href={"https://github.com/L-TED"}>
            <motion.div whileHover={{ scale: 1.25 }}>
              <svg
                width="30"
                height="30"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                transform="rotate(0 0 0)"
              >
                <path
                  d="M12 2.24902C6.51613 2.24902 2 6.70064 2 12.249C2 16.6361 4.87097 20.3781 8.87097 21.7329C9.3871 21.8297 9.54839 21.5071 9.54839 21.2813C9.54839 21.0555 9.54839 20.4103 9.51613 19.5393C6.74194 20.1845 6.16129 18.1845 6.16129 18.1845C5.70968 17.0555 5.03226 16.7329 5.03226 16.7329C4.12903 16.0877 5.06452 16.0877 5.06452 16.0877C6.06452 16.12 6.6129 17.12 6.6129 17.12C7.48387 18.6684 8.96774 18.2168 9.51613 17.9264C9.6129 17.2813 9.87097 16.8297 10.1613 16.5716C7.96774 16.3458 5.6129 15.4748 5.6129 11.6684C5.6129 10.5716 6.03226 9.70064 6.64516 9.02322C6.54839 8.79741 6.19355 7.76515 6.74194 6.37806C6.74194 6.37806 7.6129 6.11999 9.51613 7.41031C10.3226 7.18451 11.1613 7.05548 12.0323 7.05548C12.9032 7.05548 13.7742 7.15225 14.5484 7.41031C16.4516 6.15225 17.2903 6.37806 17.2903 6.37806C17.8387 7.73289 17.5161 8.79741 17.3871 9.02322C18.0323 9.70064 18.4194 10.6039 18.4194 11.6684C18.4194 15.4748 16.0645 16.3458 13.871 16.5716C14.2258 16.8942 14.5484 17.5393 14.5484 18.4426C14.5484 19.7974 14.5161 20.8619 14.5161 21.1845C14.5161 21.4426 14.7097 21.7329 15.1935 21.6361C19.129 20.3135 22 16.6039 22 12.1845C21.9677 6.70064 17.4839 2.24902 12 2.24902Z"
                  fill="#000000"
                />
              </svg>
            </motion.div>
          </Link>

          <Link href={"https://www.notion.so/4fc6bc6d782848489db6ba2882b09e16"}>
            <motion.div whileHover={{ scale: 1.25 }}>
              <svg
                width="26"
                height="26"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                transform="rotate(0 0 0)"
              >
                <path
                  d="M5.71559 5.50641C6.33766 6.01182 6.57102 5.97324 7.73915 5.89531L18.7515 5.23407C18.9851 5.23407 18.7909 5.00106 18.713 4.96234L16.8841 3.64017C16.5336 3.3681 16.0667 3.05652 15.1719 3.13446L4.5086 3.9122C4.11971 3.95077 4.04204 4.14521 4.19691 4.30106L5.71559 5.50641ZM6.37677 8.07285V19.6598C6.37677 20.2825 6.68796 20.5155 7.38834 20.477L19.4909 19.7766C20.1917 19.7381 20.2698 19.3098 20.2698 18.804V7.29475C20.2698 6.78972 20.0755 6.51735 19.6465 6.55624L6.99919 7.29475C6.53241 7.33399 6.37677 7.56747 6.37677 8.07285ZM18.3244 8.69442C18.402 9.04468 18.3244 9.39467 17.9735 9.43406L17.3904 9.55025V18.1045C16.8841 18.3766 16.4172 18.5322 16.0282 18.5322C15.4053 18.5322 15.2492 18.3376 14.7827 17.7547L10.9684 11.7667V17.5602L12.1754 17.8326C12.1754 17.8326 12.1754 18.5322 11.2016 18.5322L8.51702 18.6879C8.43903 18.5322 8.51702 18.1437 8.78933 18.0658L9.48986 17.8717V10.2115L8.51721 10.1335C8.43918 9.78323 8.63349 9.27813 9.1787 9.23893L12.0586 9.04481L16.0282 15.1108V9.74466L15.016 9.6285C14.9384 9.20026 15.2492 8.88932 15.6385 8.85075L18.3244 8.69442ZM3.61329 2.86243L14.7048 2.04561C16.0669 1.92879 16.4173 2.00705 17.2734 2.62891L20.814 5.11741C21.3982 5.54533 21.5929 5.66183 21.5929 6.12832V19.7766C21.5929 20.632 21.2813 21.1379 20.1919 21.2153L7.31126 21.9931C6.49346 22.0321 6.10428 21.9156 5.67598 21.3709L3.06867 17.988C2.60152 17.3653 2.40723 16.8994 2.40723 16.3544V4.22284C2.40723 3.52335 2.71892 2.93986 3.61329 2.86243Z"
                  fill="#000000"
                  fillRule="evenodd"
                  clipRule="evenodd"
                />
              </svg>
            </motion.div>
          </Link>
        </div>
      </div>
    </section>
  );
};

export default SectContact;
